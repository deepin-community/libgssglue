     ...
     gss_OID         type;
{
	...
	if ((__gss_mechs_array[i]->mech_type.length == type->length) &&
	    (memcmp
	     (__gss_mechs_array[i]->mech_type.elements, type->elements,
	      type->length) == 0)) {
...
OM_uint32
__gss_import_internal_name(minor_status, mech_type, union_name,
			   internal_name)
...
OM_uint32
__gss_display_internal_name(minor_status, mech_type, internal_name,
 ...
 */
OM_uint32
__gss_convert_name_to_union_name(minor_status, mech,
     ...
     gss_name_t     *external_name;
{
	...
	*minor_status = ENOMEM;
    ...
    union_name->mech_type = GSS_C_NO_OID;
